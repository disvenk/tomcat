    ...
    public static void main(String args[]) {
        ...
        synchronized (daemonLock) {
            if (daemon == null) {
                ...
                try {
                    //初始化路径,初始化类加载器,初始化Boostrap的Catalina对象
                    bootstrap.init();
                } catch (Throwable t) {
                    handleThrowable(t);
                    t.printStackTrace();
        ...
        try {
            ...
            //外部参数传入，main方法可传入的参数有 startd、stopd、start、stop，startd和start的区别是start设置了一个await的bool变量为true
            if (args.length > 0) {
